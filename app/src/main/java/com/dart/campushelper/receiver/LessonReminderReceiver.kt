...
import com.dart.campushelper.repo.DatabaseRepo
...
class LessonReminderReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        CoroutineScope(Dispatchers.IO).launch {
            val needUpdateDatabase = databaseRepo.getAll().isEmpty() || LocalTime.now().hour < 8
            if (needUpdateDatabase) {
                ...
                networkRepo.getSchedule(null)?.filter {
                    ...
                    if (it.isEmpty()) {
                        databaseRepo.insertAll(SimplifiedCourse.placeholder())
                    } else {
                        databaseRepo.insertAll(*it)
                    }
                }
            }
            val node = getCurrentBigNode(LocalTime.now().plusMinutes(AHEAD_MINUTES))
            databaseRepo.findSimplifiedCourseByBigNode(node)?.let {
                notification.show(
